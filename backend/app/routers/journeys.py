...
def get_ids_of_user_journeys(current_user: Annotated[schemas.User, Depends(get_current_user)], db: Session = Depends(get_db)):
    return db.query(models.Journey).filter(models.Journey.user_id == current_user.id).all()


@journey_router.get('/get_journeys/{user_id}', response_model=schemas.JourneyResponseModel)
def get_ids_of_user_journeys(user_id: int, current_user: Annotated[schemas.User, Depends(get_current_user)], db: Session = Depends(get_db)):
